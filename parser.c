...
void compileAssignSt(void) { // ok
  // TODO: parse the assignment and check type consistency
  Type* lvalue_type = compileLValue();
  eat(SB_ASSIGN);
  Type* expression_type = compileExpression();
  //checkTypeEquality(lvalue_type, expression_type);
  checkTypeCompatibility(lvalue_type, expression_type);
}
...
void compileArguments(ObjectNode* paramList) { //ok
  ...
  case SB_LPAR:
    ...
    while (lookAhead->tokenType == SB_COMMA) {
      ...
      if(currentParam == NULL) // if there is no comma = no more arguments and there still param in list, error
       error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);
