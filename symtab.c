...
Type* intType;
Type* charType;
Type* stringType; // mo rong
Type* doubleType; // mo rong
...
ConstantValue* duplicateConstantValue(ConstantValue* v) {
  ConstantValue* value = (ConstantValue*) malloc(sizeof(ConstantValue));
  value->type = v->type;
  if (v->type == TP_INT || v->type == TP_DOUBLE) // mo rong
    value->numValue = v->numValue;
...
void initSymTab(void) {
  ...
  stringType = makeStringType(); // mo rong
  doubleType = makeDoubleType(); // mo rong
}
...
void cleanSymTab(void) {
  ...
  freeType(intType);
  freeType(charType);
  freeType(stringType); // mo rong
  freeType(doubleType); // mo rong
}
