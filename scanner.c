...
Token* readNumber(void) {
  ...
  while(currentChar != EOF && (charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_PERIOD)){
    ...
    if(charCodes[currentChar] == CHAR_PERIOD) {
      if(isFloat > 1) error(ERR_INVALID_DOUBLE, token->lineNo, token->colNo);
      isFloat++;
    }
...
Token* readPointFloat(){
  ...
  while(currentChar != EOF && (charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_PERIOD)){
    if(index >= MAX_IDENT_LEN){
      while(currentChar != EOF && (charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_PERIOD))
        readChar();
      error(ERR_INVALID_DOUBLE, lineNo, colNo);
      return token;
    }
    str[index++] = currentChar;
    readChar();
  ...
  str[index] = '\0';
  strcpy(token->string, str);
  token->value = atof(token->string); // mo rong
  token->tokenType = TK_DOUBLE;
  free(str);
  return token;
}
...
Token* readString(void){
  Token* token = makeToken(TK_NONE, lineNo, colNo);
  free(token->string); // mo rong
  token->string = (char*)malloc(MAX_STRING_LEN+1); // mo rong
  char* str = (char*)malloc(MAX_STRING_LEN+1);
  int index = 0;
  readChar();
  while(currentChar != EOF && charCodes[currentChar] != CHAR_DOUBLEQUOTE){
    if(index >= MAX_STRING_LEN){
      while(currentChar != EOF && charCodes[currentChar] != CHAR_DOUBLEQUOTE)
        readChar();
      error(ERR_INVALID_STRING, lineNo, colNo);
      return token;
    }
    str[index++] = currentChar;
    readChar();
...
Token* getToken(void) {
  ...
  case CHAR_EXCLAIMATION:
    ...
    if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) {
      ...
      return makeToken(SB_FACTORIAL, ln, cn);
  case CHAR_COMMA:
...
void printToken(Token *token) {
  ...
  case SB_FACTORIAL: printf("SB_FACTORIAL\n"); break;
  case KW_SWITCH: printf("KW_SWTICH\n"); break;
